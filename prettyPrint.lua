...
local function isPrimitiveArray(array)
  ...
  for k, v in pairs(array) do
    local isKeyOk = (type(k) == 'number'
        and k > 0 and math.floor(k) == k)
    if not isKeyOk or not isPrimitiveType[type(v)] then
      return false
    end
  ...
  for i = 1, #keyList do
    ...
    if type(value) == 'table' then
      if tableRef[value] == nil then -- prevent reference loops
        ...
        for k, v in pairs(value) do
          if isPrimitiveType[type(k)]
                and (isPrimitiveType[type(v)] or type(v) == 'table')
          then
            has_items = true
            break
          end
    ...
    elseif isPrimitiveType[type(value)] then
      output = output .. indentStr ..
          formatKey(key, in_seq) .. formatValue(value) .. " \n"
    end
  end
  return output
end

local prettyPrint = function(dataTable)
  local s = traverseTable(dataTable, {[dataTable] = true}, 1)
  return "{\n" .. s .. "}"
end
